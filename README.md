...
如Collections类中的静态内部类，方法声明中的类型是公共接口，但是实际返回子类型。
4. 静态工厂方法在创建参数化类型实例的时候，代码变得更加简洁
...
要优先使用基本类型而不是装箱基本类型，避免无意识的自动装箱
### 6：消除过期的对象引用
当我们不需要一个对象时，可以将其设置为null。但是清空对象引用应该是一种例外，而不是一种规范行为。只要是类自己管理内存，程序员就应该警惕内存泄漏问题。
### 7：避免使用终结方法
1. 终结方法的缺点是不能保证此方法会被及时的执行。
2. 终结方法并不一定会执行。假设我们依赖终结方法来释放共享资源（比如数据库上的永久锁），很容易让整个分布式系统垮掉。
3. 如果在终结方法执行过程中，有未被捕获的异常抛出，则这种异常会被忽略，并且该对象的终结过程也会终止。
4. 终结方法会有非常严重的性能损失
